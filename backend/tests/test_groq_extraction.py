...
5. extract_intelligence() tries Groq before Gemini when both keys happen to be set.
"""
...
@pytest.mark.asyncio
async def test_extract_intelligence_tries_groq_before_gemini(monkeypatch):
    """When both GROQ_API_KEY and GEMINI_API_KEY are set, Groq is tried first."""
    monkeypatch.setenv("GROQ_API_KEY", "fake-groq-key")
    monkeypatch.setenv("GEMINI_API_KEY", "fake-gemini-key")

    claim_payload = {
        "category": "REPORT",
        "content": "Groq handled this.",
        "confidence": 0.9,
        "evidence_status": "REPORTED",
    ...
    gemini_call = AsyncMock(side_effect=AssertionError("Gemini should not have been called"))

    with patch("app.engine.extraction.httpx.AsyncClient", return_value=_mock_async_client(mock_response)), \
         patch("app.engine.extraction.extract_with_gemini", gemini_call):
        result = await extract_intelligence("The main pump is failing.", speaker="Field Tech")

    assert result.extraction_method == "llm"
    assert result.content == "Groq handled this."
    gemini_call.assert_not_called()


@pytest.mark.asyncio
async def test_extract_intelligence_falls_through_to_gemini_when_groq_unset(monkeypatch):
    """Without GROQ_API_KEY, extract_intelligence falls through to Gemini as before."""
    monkeypatch.setenv("GROQ_API_KEY", "")
