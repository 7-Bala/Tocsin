...
async def get_weather_risk(
    ...
    try:
        ...
        logger.error(f"[MCP TOOL RESULT] tool=get_weather_risk error={exc} isError=True")
        return make_evidence_envelope(
            ...
            extra_root_fields={"error": f"Failed to retrieve weather data: {exc}", "risk_label": "UNKNOWN"},
        )
...
async def find_nearby_resource(
    ...
    try:
        ...
        return make_evidence_envelope(
            ...
            source_type="MAPPED",
            summary=summary,
            data={"found": True, "resource_type": resource_type, "nearest": nearest, "candidate_count": len(matches)},
            location={"latitude": latitude, "longitude": longitude, "radius_km": radius_km},
            confidence="COMMUNITY_MAPPED",
            limitations=[
        ...
        return make_evidence_envelope(
            ...
            extra_root_fields={"found": False, "error": f"Resource lookup failed: {exc}", "resource_type": resource_type},
        )
    ...
    try:
        ...
        if routes:
            ...
            return make_evidence_envelope(
                ...
                source_type="MODEL",
                summary=summary,
                data={"distance_km": dist_km, "duration_minutes": dur_min, "mode": mode, "estimated": False},
                location={"origin": [origin_lat, origin_lng], "destination": [dest_lat, dest_lng]},
        ...
        return make_evidence_envelope(
            ...
            source_type="MODEL",
            summary=summary,
            data={"distance_km": dist_km, "duration_minutes": est_duration_min, "mode": mode, "estimated": True},
            location={"origin": [origin_lat, origin_lng], "destination": [dest_lat, dest_lng]},
...
async def propose_incident_action(
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            ...
            if resp.status_code == 404:
                return make_evidence_envelope(
                    ...
                    extra_root_fields={"proposed": False, "error": f"Incident '{incident_id}' not found.", "status_code": 404},
                )
            resp.raise_for_status()
            data = resp.json()
        ...
        return make_evidence_envelope(
            ...
            extra_root_fields={"proposed": False, "error": f"Failed to propose action: {exc}", "incident_id": incident_id},
        )
...
async def get_earthquake_activity(
    ...
    try:
        ...
        for feat in features:
            ...
            eq_lon, eq_lat, depth_km = coords[0], coords[1], coords[2] if len(coords) > 2 else 0.0
            ...
            events.append({
                ...
                "time_utc": datetime.fromtimestamp(props.get("time", 0) / 1000.0, timezone.utc).isoformat() if props.get("time") else None,
                "url": props.get("url"),
            })

        max_mag = max([e["magnitude"] for e in events if e.get("magnitude") is not None], default=0.0)
        risk_level = "HIGH" if max_mag >= 6.0 else "MODERATE" if max_mag >= 4.5 else "LOW" if events else "NONE_DETECTED"
        ...
        return make_evidence_envelope(
            ...
            extra_root_fields={
                "search_center": {"latitude": latitude, "longitude": longitude, "radius_km": radius_km},
                "events_count": len(events),
                "events": events,
                "max_magnitude": max_mag,
                "seismic_risk_level": risk_level,
        ...
        return make_evidence_envelope(
            ...
            extra_root_fields={"error": f"Failed to retrieve USGS earthquake data: {exc}", "events_count": 0, "events": []},
        )
...
async def get_active_fire_hotspots(
    ...
    if map_key and map_key.strip():
        ...
        try:
                    ...
                    if len(parts) >= len(headers):
                        ...
                        if dist_km <= radius_km:
                            hotspots.append({
                                ...
                                "longitude": h_lon,
                                "distance_km": dist_km,
                                "brightness_kelvin": float(row.get("bright_ti4", 0.0) or 0.0),
            ...
            return make_evidence_envelope(
                ...
                summary=summary,
                data={"hotspots_count": len(hotspots), "hotspots": hotspots[:10], "instrument": "VIIRS / MODIS Near-Real-Time"},
                location={"latitude": latitude, "longitude": longitude, "radius_km": radius_km},
    ...
    return make_evidence_envelope(
        ...
        source_type="OBSERVATIONAL",
        summary=summary,
        data={"hotspots_count": 0, "hotspots": [], "instrument": "VIIRS / MODIS Near-Real-Time"},
        location={"latitude": latitude, "longitude": longitude, "radius_km": radius_km},
        confidence="SATELLITE_THERMAL_INFRARED",
        limitations=[
...
async def get_official_emergency_alerts(
    ...
    if cache_key in _ALERT_CACHE:
        ...
        if now - cached_time < _CACHE_TTL_SECONDS:
            logger.info(f"Returning cached official alerts for ({latitude}, {longitude})")
            return cached_val
    ...
    if is_us_region:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0, headers={"User-Agent": USER_AGENT, "Accept": "application/geo+json"}) as client:
                resp = await client.get(url)
                if resp.status_code == 200:
                    ...
                    res = make_evidence_envelope(
                        ...
                        source_type="OFFICIAL",
                        summary=summary,
                        data={"active_alerts_count": len(alerts), "alerts": alerts, "coverage_region": "UNITED STATES"},
                        location={"latitude": latitude, "longitude": longitude},
                        ...
                        extra_root_fields={"active_alerts_count": len(alerts), "alerts": alerts},
                    )
    ...
    if is_india_region:
        ...
        try:
            async with httpx.AsyncClient(timeout=6.0, verify=False, headers={"User-Agent": USER_AGENT, "Content-Type": "application/json"}) as client:
                # 1. Primary: Query SACHET Location-wise CAP API
                url_loc = f"{sachet_base}/FetchLocationWiseAlerts"
                resp_loc = await client.post(url_loc, params={"lat": latitude, "long": longitude, "radius": "50"})
                if resp_loc.status_code == 200:
                    raw_loc = resp_loc.json().get("alerts", [])
                    for a in raw_loc:
                        alerts.append({
                            "event": a.get("disaster_type") or a.get("events") or "Disaster Alert",
                            "headline": f"{a.get('disaster_type', 'Alert')} - {a.get('area_description', 'India')}",
                            "severity": a.get("severity") or a.get("severity_color", "ALERT"),
                            ...
                            "sender_name": a.get("alert_source") or "NDMA / IMD India",
                ...
                if not alerts:
                    ...
                    if resp_all.status_code == 200:
                        ...
                        for a in all_items:
                            ...
                            if "chennai" in area_desc and haversine_distance_km(latitude, longitude, 13.0827, 80.2707) <= 150.0:
                                is_match = True

                            if is_match:
                                alerts.append({
                                    "event": a.get("disaster_type") or "Disaster Alert",
                ...
                res = make_evidence_envelope(
                    ...
                    source_type="OFFICIAL",
                    summary=summary,
                    data={"active_alerts_count": len(alerts), "alerts": alerts, "coverage_region": "INDIA (NATIONAL & STATE CAP)"},
                    location={"latitude": latitude, "longitude": longitude},
                    confidence="GOVERNMENT_AUTHORITATIVE",
                    limitations=[
            ...
            if resp.status_code == 200:
                ...
                for item in items[:15]:
                    ...
                    alerts.append({
                        "event": title,
                        "headline": title,
                        "severity": "Severe" if "red" in title.lower() or "orange" in title.lower() else "Moderate",
                        "urgency": "Expected",
                        "certainty": "Observed",
                ...
                res = make_evidence_envelope(
                    ...
                    summary=summary,
                    data={"active_alerts_count": len(alerts), "alerts": alerts, "coverage_region": "GLOBAL / INTERNATIONAL"},
                    location={"latitude": latitude, "longitude": longitude},
...
async def get_global_disaster_alerts(
    ...
    try:
        ...
        return make_evidence_envelope(
            ...
            source_type="AGGREGATED",
            summary=summary,
            data={"global_alerts_count": len(alerts), "alerts": alerts, "total_feed_items": len(items)},
            location={},
            confidence="MULTI_AGENCY_AGGREGATED",
            limitations=[
        ...
        return make_evidence_envelope(
            ...
            extra_root_fields={"global_alerts_count": 0, "alerts": [], "error": f"Failed to retrieve GDACS disaster feed: {exc}"},
        )
...
async def get_air_quality_hazards(
    ...
    try:
        ...
        return make_evidence_envelope(
            ...
            extra_root_fields={"error": f"Failed to retrieve air quality data: {exc}"},
        )
...
async def search_emergency_infrastructure(
    ...
    for el in elements:
        ...
        if el_lat is not None and el_lon is not None:
            dist_km = haversine_distance_km(latitude, longitude, float(el_lat), float(el_lon))
            tags = el.get("tags", {})
            name = tags.get("name") or tags.get("description") or f"Unnamed {type_normalized.title()}"
            items.append({
                "name": name,
    ...
    if not items:
        nom_result = await find_nearby_resource(latitude=latitude, longitude=longitude, resource_type=type_normalized, radius_km=radius_meters / 1000.0)  # type: ignore[arg-type]
    ...
    return make_evidence_envelope(
        ...
        source_type="MAPPED",
        summary=summary,
        data={"infrastructure_type": type_normalized, "facilities_found": len(items), "facilities": items[:10]},
