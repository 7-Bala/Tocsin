...
| **Spoken Audio Summary Broadcast** | `IMPLEMENTED — CREDENTIAL REQUIRED` | `POST /api/agora/speak` calls Agora's documented `/speak` endpoint against an already-running agent, wired to a "🔊 Broadcast" button on the handoff brief's spoken script. Request shape verified by regression tests and curl'd against real credentials (correct 404 with no active agent); actual audio delivery in a live channel not yet observed — requires a live paid session. See `docs/agora/RESEARCH.md` §10. |
...
| **MCP Tool Calling During Live Voice Sessions** | `IMPLEMENTED — CREDENTIAL REQUIRED` | The 13 emergency-intelligence tools are real and independently callable (`mock-services/server.py`). Default voice pipeline (`gemini_live`, Agora's `mllm`) confirmed **not** to support `mcp_servers` per official docs, and Tocsin never sends it there. A second pipeline, `voice_pipeline: "composed_tools"` on `/start-agent`, wires MCP tools per Agora's actually-documented `llm.mcp_servers` schema — Gemini stays the reasoning model (BYOK, same key), ASR/TTS use Agora-managed credentials (no new third-party keys). Payload shape verified against a mocked Agora response in tests; real Agora acceptance and actual tool invocation both require a live credentialed session, not yet performed. Trades Gemini Live's single-hop native audio for a 3-hop ASR→LLM→TTS pipeline, so it's opt-in, never the default. See `docs/agora/RESEARCH.md` §4. |
